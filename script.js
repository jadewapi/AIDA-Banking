...
transferArrow.addEventListener("click", () => {
  if (currentAccount) {
    ...
    if (currentAccount != transferToAccount) {
      if (
        Number(amount.value) <= currentAccount.currentBalance &&
        amount.value != "" &&
        amount.value > 0
      ) {
...
requestArrow.addEventListener("click", () => {
  if (currentAccount) {
    const boolean = currentAccount.movementInfo.some(
      (movementObj) =>
        movementObj.movementAmount >= Math.abs(Number(loan.value) * 0.1) &&
...
function calculateCurrentBalance(allAccounts) {
  allAccounts.forEach((accountObj) => {
    const movementAmountArray = accountObj.movementInfo.map(
      (movementObj) => movementObj.movementAmount
    );
...
function displayTransactionMovements(currentAccount, sort = false) {
  ...
  const copiedObjectForSort = {
    ...currentAccount,
    movementInfo: currentAccount.movementInfo
      .slice()
      .sort((a, b) => a.movementAmount - b.movementAmount),
  };
  ...
  copiedOrOriginal.movementInfo.forEach((movementObj, index) => {
    const html = `<div class="specificTransaction">
            <div class="transactionInfo">
                <p class="transactionType ${
                  movementObj.movementAmount > 0 ? "deposit" : "withdraw"
                }">${index + 1}: ${
      movementObj.movementAmount > 0 ? "DEPOSIT" : "WITHDRAW"
    }</p>
